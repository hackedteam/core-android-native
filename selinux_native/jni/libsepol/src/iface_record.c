			   ...
			   const char *name, sepol_iface_key_t ** key_ptr)
{
	...
	if (!tmp_key) {
		LOGD("out of memory, could not create interface key");
		return STATUS_ERR;
	}
			    ...
			    sepol_iface_key_t ** key_ptr)
{
	...
	if (sepol_iface_key_create(handle, iface->name, key_ptr) < 0) {
		LOGD("could not extract key from "
...
int sepol_iface_create(sepol_handle_t * handle, sepol_iface_t ** iface)
{
	...
	if (!tmp_iface) {
		LOGD("out of memory, could not create "
		    "interface record");
		return STATUS_ERR;
	}
			 ...
			 sepol_iface_t * iface, const char *name)
{
	...
	if (!tmp_name) {
		LOGD("out of memory, " "could not set interface name");
		return STATUS_ERR;
	}
	free(iface->name);
	iface->name = tmp_name;
			  ...
			  sepol_iface_t * iface, sepol_context_t * con)
{
	...
	if (sepol_context_clone(handle, con, &newcon) < 0) {
		LOGD("out of memory, could not set interface context");
		return STATUS_ERR;
	}
			   ...
			   sepol_iface_t * iface, sepol_context_t * con)
{
	...
	sepol_context_t *newcon;
	if (sepol_context_clone(handle, con, &newcon) < 0) {
		LOGD("out of memory, could not set message context");
		return STATUS_ERR;
	}
		      ...
		      const sepol_iface_t * iface, sepol_iface_t ** iface_ptr)
{
	...
	*iface_ptr = new_iface;
	return STATUS_SUCCESS;
      ...
      err:
	LOGD("could not clone interface record");
