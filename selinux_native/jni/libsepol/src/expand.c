			      ...
			      void *data)
{
	...
	if (state->verbose)
	  LOGD("copying type or attribute %s", id);
				 ...
				 void *data)
{
	...
	if (state->verbose)
		LOGD("converting attribute %s", id);

	new_type = hashtab_search(state->out->p_types.table, id);
	if (!new_type) {
		LOGD( "attribute %s vanished!", id);
		return -1;
	}
			...
			if (new_expr->expr_type == CEXPR_NAMES) {
				if (new_expr->attr & CEXPR_TYPE) {
					...
					if (map_ebitmap(&expr->type_names->types,
							...
							state->typemap)) {
						LOGD( "Failed to map type_names->types");
						goto out_of_mem;
					}
					 ...
					 class_datum_t *newdatum)
{
	if (olddatum->default_user) {
		if (newdatum->default_user && olddatum->default_user != newdatum->default_user) {
			LOGD( "Found conflicting default user definitions");
	...
	if (olddatum->default_role) {
		if (newdatum->default_role && olddatum->default_role != newdatum->default_role) {
			LOGD( "Found conflicting default role definitions");
			return SEPOL_ENOTSUP;
		}
	...
	if (olddatum->default_type) {
		if (newdatum->default_type && olddatum->default_type != newdatum->default_type) {
			LOGD( "Found conflicting default type definitions");
			return SEPOL_ENOTSUP;
		}
	...
	if (olddatum->default_range) {
		if (newdatum->default_range && olddatum->default_range != newdatum->default_range) {
			LOGD( "Found conflicting default range definitions");
			return SEPOL_ENOTSUP;
		}
			       ...
			       void *data)
{
	...
	if (class->comkey) {
		...
		if (!new_class->comdatum) {
			LOGD( "could not find common datum %s",
			    new_class->comkey);
			return -1;
		}
				     ...
				     hashtab_datum_t datum, void *data)
{
	...
	if (!dest) {
		LOGD( "Type lookup failed for %s", (char *)key);
		return -1;
	}
	if (dest->bounds != 0 && dest->bounds != bounds_val) {
		LOGD( "Inconsistent boundary for %s", (char *)key);
		return -1;
	}
				     ...
				     hashtab_datum_t datum, void *data)
{
	...
	if (!dest) {
		LOGD( "Role lookup failed for %s", (char *)key);
		return -1;
	}
	if (dest->bounds != 0 && dest->bounds != bounds_val) {
		LOGD( "Inconsistent boundary for %s", (char *)key);
				     ...
				     hashtab_datum_t datum, void *data)
{
	...
	if (!dest) {
		LOGD( "User lookup failed for %s", (char *)key);
		return -1;
	}
	if (dest->bounds != 0 && dest->bounds != bounds_val) {
		LOGD( "Inconsistent boundary for %s", (char *)key);
			     ...
			     void *data)
{
	...
	if (state->verbose)
		LOGD("fixing role attribute %s", id);
			      ...
			      policydb_t * p, sepol_handle_t * h)
{
	...
	if (!levdatum) {
		LOGD( "%s: Impossible situation found, nothing in p_levels.table.\n",
		    __func__);
		errno = ENOENT;
		return -1;
	}
	for (cat = sl->cat; cat; cat = cat->next) {
		if (cat->low > cat->high) {
			LOGD( "Category range is not valid %s.%s",
		...
		for (i = cat->low - 1; i < cat->high; i++) {
			if (!ebitmap_get_bit(&levdatum->level->cat, i)) {
				LOGD( "Category %s can not be associate with "
				    "level %s",
			      ...
			      policydb_t * p, sepol_handle_t * h)
{
	...
	if (!mls_level_dom(&r->level[1], &r->level[0])) {
		mls_range_destroy(r);
		LOGD( "MLS range high level does not dominate low level");
		return -1;
	}
			      ...
			      void *data)
{
	...
	if (!new_user) {
		...
		if (!mls_level_between(&new_user->exp_dfltlevel,
			...
			LOGD( "default level not within user "
			    "range");
			return -1;
		}
	} else {
			      ...
			      void *data)
{
	...
	if (state->verbose)
		LOGD("copying sensitivity level %s", id);
			      ...
			      void *data)
{
	...
	if (state->verbose)
		LOGD("copying category attribute %s", id);
				...
				while (cur_trans != NULL) {
					if ((cur_trans->ttype == j + 1) &&
						...
						if (cur_trans->otype == mapped_otype)
							break;
						LOGD( "Conflicting filename trans rules %s %s %s : %s otype1:%s otype2:%s",
						    cur_trans->name,
			      ...
			      range_trans_rule_t * rules)
{
	...
	if (state->verbose)
		LOGD("expanding range transitions");
...
static int cond_node_map_bools(expand_state_t * state, cond_node_t * cn)
{
	...
	if (cond_normalize_expr(state->out, cn)) {
		LOGD( "Error while normalizing conditional");
		return -1;
	}
