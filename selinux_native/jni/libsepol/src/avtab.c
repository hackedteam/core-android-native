...
void avtab_hash_eval(avtab_t * h, char *tag)
{
	...
	LOGD("%s:  %d entries and %d/%d buckets used, longest chain length %d\n",
				    ...
				    avtab_datum_t * d, void *p), void *p)
{
	...
	if (vers < POLICYDB_VERSION_AVTAB) {
		...
		if (rc < 0) {
			LOGD("truncated entry");
			return -1;
		}
		items2 = le32_to_cpu(buf32[0]);

		if (items2 < 5 || items2 > ARRAY_SIZE(buf32)) {
			LOGD("invalid item count");
			return -1;
		}
		...
		if (key.source_type != val) {
			LOGD("truncated source type");
			return -1;
		}
		val = le32_to_cpu(buf32[items++]);
		key.target_type = (uint16_t) val;
		if (key.target_type != val) {
			LOGD("truncated target type");
			return -1;
		}
		val = le32_to_cpu(buf32[items++]);
		key.target_class = (uint16_t) val;
		if (key.target_class != val) {
			LOGD("truncated target class");
			return -1;
		}
	...
	for (i = 0; i < nel; i++) {
		...
		if (rc) {
			if (rc == SEPOL_ENOMEM)
				LOGD("out of memory");
			if (rc == SEPOL_EEXIST)
				LOGD("duplicate entry");
			LOGD("failed on entry %d of %u", i, nel);
			goto bad;
		}
	}
