			  ...
			  sepol_port_key_t ** key_ptr)
{
	...
	if (!tmp_key) {
		LOGD( "out of memory, could not create " "port key");
		return STATUS_ERR;
	}
			   ...
			   sepol_port_key_t ** key_ptr)
{
	...
	if (sepol_port_key_create
		...
		LOGD( "could not extract key from port %s %d:%d",
...
int sepol_port_create(sepol_handle_t * handle, sepol_port_t ** port)
{
	...
	if (!tmp_port) {
		LOGD( "out of memory, could not create " "port record");
		return STATUS_ERR;
	}
		     ...
		     const sepol_port_t * port, sepol_port_t ** port_ptr)
{
	...
	*port_ptr = new_port;
	return STATUS_SUCCESS;
      ...
      err:
	LOGD( "could not clone port record");
		       ...
		       sepol_port_t * port, sepol_context_t * con)
{
	...
	if (sepol_context_clone(handle, con, &newcon) < 0) {
		LOGD( "out of memory, could not set port context");
		return STATUS_ERR;
	}
