			       ...
			       void *data)
{
	...
	if (target_type == NULL) {
		LOGD("%s: Could not find type %s for alias %s.",
		    state->cur_mod_name, target_id, id);
		return -1;
	}

	if (!strcmp(id, target_id)) {
		LOGD("%s: Self aliasing of %s.",
		    state->cur_mod_name, id);
		return -1;
	}
...
static int enable_avrules(link_state_t * state, policydb_t * pol)
{
	...
	if (state->verbose) {
		LOGD( "Determining which avrules to enable.");
	}
				  ...
				  void * data)
{
	...
	if (state->verbose)
		LOGD( "expanding role attribute %s", id);
...
static int populate_roleattributes(link_state_t *state, policydb_t *pol)
{
	...
	if (state->verbose)
		LOGD( "Populating role-attribute relationship "
