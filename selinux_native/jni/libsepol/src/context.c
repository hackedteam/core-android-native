		      ...
		      char **result, size_t * result_len)
{
	...
	*result_len = scontext_len;
	return STATUS_SUCCESS;
      ...
      omem:
	LOGD("out of memory, could not convert context to string");
	free(scontext);
	return STATUS_ERR;
}
			...
			const sepol_context_t * record)
{
	...
	usrdatum = (user_datum_t *) hashtab_search(policydb->p_users.table,
						   (hashtab_key_t) user);
	if (!usrdatum) {
		LOGD("user %s is not defined", user);
		goto err_destroy;
	}
	...
	roldatum = (role_datum_t *) hashtab_search(policydb->p_roles.table,
						   (hashtab_key_t) role);
	if (!roldatum) {
		LOGD("role %s is not defined", role);
		goto err_destroy;
	}
	...
	if (!typdatum || typdatum->flavor == TYPE_ATTRIB) {
		LOGD("type %s is not defined", type);
		goto err_destroy;
	}
	...
	/* MLS */
	if (mls && !policydb->mls) {
		LOGD("MLS is disabled, but MLS context \"%s\" found",
		    mls);
		goto err_destroy;
	} else if (!mls && policydb->mls) {
		LOGD("MLS is enabled, but no MLS context found");
		goto err_destroy;
	}
	...
	*cptr = scontext;
	...
	free(type);
	free(role);
	LOGD("could not create context structure");
	return STATUS_ERR;
}
		      ...
		      sepol_context_t ** record)
{
	...
	*record = tmp_record;
	return STATUS_SUCCESS;
      ...
      err:
	LOGD("could not create context record");
